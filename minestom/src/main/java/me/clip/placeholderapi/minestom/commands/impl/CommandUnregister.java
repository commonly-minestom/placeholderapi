...
public final class CommandUnregister extends Command {

    @NotNull
    private final PlaceholderAPIPlugin plugin;

    public CommandUnregister(@NotNull final PlaceholderAPIPlugin plugin) {
        super("unregister");
        ...
        setDefaultExecutor((sender, context) -> {
            ...
            if (args.length == 0) {
                sender.sendMessage(Component.text("Usage: /papi unregister <identifier>",
                        NamedTextColor.RED));
                return;
            }
    ...
    public void run(@NotNull final CommandSender sender, @NotNull final String identifier) {
        if (plugin.getLocalExpansionManager().unregister(identifier)) {
            sender.sendMessage(Component.text("Unregistered expansion " + identifier,
